...
%      rzero{k}  = gamrnd(hzero + U_zero{k},1./max(c_0-log(max(1-q_zero{k},eps)),eps));
  ...
  for k = 1:length(n_j_t_k)
    marginal_log_likelihood{k}(combinationIdx) = log(sum(exp(likelihood_value{k} + max(abs(likelihood_value{k}))))) - max(abs(likelihood_value{k})) - log(length(likelihood_value{k}));
